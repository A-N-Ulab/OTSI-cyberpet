...
int setting = 0, settings = 0; 
int bezelPos = 0;
int lastButtonState1, currentButtonState1, lastButtonState2, currentButtonState2, lastButtonState3, currentButtonState3;
int devState1 = LOW;
//stats
int happines = 100;
int hp = 100;
...
void core1( void * pvParameters ) {
  for(;;){
    ...
    //to devTools
    else if(digitalRead(BT3) == LOW && current_screen == 7 && settings == 3 && devTools){
      if (millis() - lastButtonPress > 50) {
        current_screen=8;
	  	  }
  		lastButtonPress = millis();
    ...
    //to Settings
    else if(btnStateE == LOW && item_selected==5){
      if (millis() - lastButtonPress > 50) {
        fix_2();
        current_screen=7;
	  	  }
    ...
    if (current_screen == 4) {
      //------start button------
      if(digitalRead(BT3) == LOW && current_screen == 4){
        if (millis() - lastButtonPress > 50) {
          time_for = setting * 60;
          button = time_for;
	  	  }
      ...
      //------encoder------
        ...
        display2.clearDisplay();
        display2.drawBitmap(0, 0, (const unsigned char *)frames, 128, 64, WHITE); 
        display2.drawBitmap(5, 54, (const unsigned char *)ret_enc, 64, 8, WHITE);  
        display2.drawBitmap(64, 55, (const unsigned char *)start, 32, 8, WHITE);
        //minutes
    ...
    if (current_screen == 8) {
      ...
      currentButtonState1 = digitalRead(BT3);

      if(lastButtonState1 == HIGH && currentButtonState1 == LOW && current_screen == 8 && devTools){
        if (millis() - lastButtonPress > 50) {
          if(devState1){
            onoff_sleep = "OFF";
            sleeping = false; 
      ...
      display2.setCursor(5, 5);
      display2.println("Btn1 - sleep: ");
      display2.setCursor(85, 5);
