...
//------variables------
//menu
const int NUM_ITEMS = 6, MAX_ITEM_LENGTH = 20;
...
//timer
int time_m = 0, time_s = 0, time_for = 0, time_for_2 = 0;
int button = 0;
int first_m = 0;
//dev tools
...
void core1( void * pvParameters ) {
  for(;;){
    ...
    //********************Encoder button for changing screens and return button********************
    ...
    //to time settings
    if(btnStateE == LOW && current_screen == 7 && settings == 1){
      if (millis() - lastButtonPress > 50){
        fix_2();
        current_screen=9;
	  	  }
    ...
    //to main menu
    else if(digitalRead(BT3) == LOW && current_screen != 1 && current_screen != 8 && current_screen != 0){
      if (millis() - lastButtonPress > 50) {
        fix_1();
        current_screen = 1;
	  	  } 
    ...
    //to settings
    else if(digitalRead(BT3) == LOW && current_screen >= 8 && current_screen <= 10){
      if (millis() - lastButtonPress > 50) {
        fix_1();
        current_screen = 7;
	  	  } 
    ...
    //to Volt/Ammeter
    else if(btnStateE == LOW && current_screen == 1 && item_selected == 4){
      if (millis() - lastButtonPress > 50) {
        fix_1();
        current_screen = 2;
		    }
